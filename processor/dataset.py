...
class MMREProcessor(object):
    ...
    def load_from_file(self, mode="train", sample_ratio=1.0):
        ...
        """
        load_file = self.data_path[mode]#'data/RE_data/txt/ours_train.txt'
        ...
        # aux image 视觉工具包提取出来的对象,可视为object-level 
        aux_path = self.data_path[mode+"_auximgs"]# e.g.: 'train_auximgs': 'data/RE_data/txt/mre_train_dict.pth'
        aux_imgs = torch.load(aux_path)#dict 12246: ['12246_pred_yolo_crop_27745.png', '12246_pred_yolo_crop_18857.png'] 按照字典列表进行排序

        # fasterrcnn object-level feature
        # imgfea_path = self.data_path[mode+"_imgfeas"]
        # with open(imgfea_path, 'rb') as f:
        #     img_features = pickle.load(f)
       
     
        # sample
        if sample_ratio != 1.0:
            sample_indexes = random.choices(list(range(len(words))), k=int(len(words)*sample_ratio))# [0,1,2,, ... ,len(words)]  k=int
    ...
    def get_relation_dict(self):
        with open(self.re_path, 'r', encoding="utf-8") as f: #'data/RE_data/ours_rel2id.json'
            line = f.readlines()[0]
...
class MMREDataset(Dataset):
    def __init__(self, processor, transform, img_path=None, aux_img_path=None, img_objfea=None, max_seq=40, sample_ratio=1.0, mode="train") -> None:
        ...
        self.img_features_path = img_objfea[mode]
    ...
    def __getitem__(self, idx):
        ...
        item_id = self.data_dict['dataid'][idx]# dataid定位对应的aux_img
        ...
        extend_word_list = " ".join(extend_word_list)#list to str
        encode_dict = self.tokenizer.encode_plus(text=extend_word_list, max_length=self.max_seq, truncation=True, padding='max_length')#tokenizer
        ...
        #头尾实体
        head_entity = head_d['name']
        tail_entity = head_d['name']
        head_enc = self.tokenizer.encode_plus(text=head_entity, max_length=10, truncation=True, padding='max_length')
        tail_enc = self.tokenizer.encode_plus(text=tail_entity, max_length=10, truncation=True, padding='max_length')
