...
# -----------------------------------------------------------------------------------------
class RectifiedActivationUnit(nn.Module):
    def __init__(self, args, num_out_path):
        super(RectifiedActivationUnit, self).__init__()
        self.keep_mapping = nn.ReLU()
...
# -----------------------------------------------------------------------------------------
class FeatureSemanticReasoningUnit(nn.Module):
    def __init__(self, opt, num_out_path):
        super(FeatureSemanticReasoningUnit, self).__init__()
        self.opt = opt
        self.router = Router(num_out_path, opt.embed_size, opt.hid_router)
        self.sa = SelfAttention(opt.embed_size, opt.hid_FSRU, opt.num_head_FSRU)
