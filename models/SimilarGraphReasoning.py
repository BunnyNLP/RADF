...
class VisualSA(nn.Module):
    ...
    Args: - local: local region embeddings, shape: (batch_size, 4, 1024)
    ...
    def forward(self, local, raw_global):
        ...
        l_emb = self.embedding_local(local)#torch.Size([16, 3, 1024])
        g_emb = self.embedding_global(raw_global)#torch.Size([16, 1024])

        # compute the normalized weights, shape: (batch_size, 36)
        g_emb = g_emb.unsqueeze(1).repeat(1, l_emb.size(1), 1)# torch.Size([16, 3, 1024])
        common = l_emb.mul(g_emb)#element-wise multiple -> torch.Size([16, 3, 1024])
        weights = self.embedding_common(common).squeeze(2)#torch.Size([16, 3])
        weights = self.softmax(weights)#torch.Size([16, 3])
...
class TextSA(nn.Module):
    ...
    def forward(self, local, raw_global):#torch.Size([1, 24, 1024]), torch.Size([1, 1024])
        # compute embedding of local words and raw global text
        l_emb = self.embedding_local(local)#torch.Size([1, 24, 1024]) -> torch.Size([1, 24, 1024])
        g_emb = self.embedding_global(raw_global)# torch.Size([1, 1024]) -> torch.Size([1, 1024])
...
class EncoderSimilarity(nn.Module):
    ...
    """
    def __init__(self, embed_size=1024, sim_dim=256,  sgr_step=3):
        super(EncoderSimilarity, self).__init__()

        self.v_global_w = VisualSA(embed_size, 0.4, 3)
    ...
    def forward(self, img_emb, cap_emb, cap_lens):
        ...
        sim_all = []
        n_image = img_emb.size(0)#16
        n_caption = cap_emb.size(0)#16

        # get enhanced global images by self-attention
        img_ave = torch.mean(img_emb, 1)# torch.Size([16, 3, 1024]) -> torch.Size([16, 1024]) Todo:如果效果差尝试换resnet
        img_glo = self.v_global_w(img_emb, img_ave)

        for i in range(n_caption):
            # get the i-th sentence
            n_word = cap_lens[i]#get the length of the i-th sentence
            cap_i = cap_emb[i, :n_word, :].unsqueeze(0)#get the word embed of i-th sentence ->torch.Size([1, n_word, 1024])
            cap_i_expand = cap_i.repeat(n_image, 1, 1)#repeat bzs -> torch.Size([16, n_word, 1024])

            # get enhanced global i-th text by self-attention
            cap_ave_i = torch.mean(cap_i, 1)# torch.Size([1, 1024]) Todo:如果效果差尝试换cls输出
            cap_glo_i = self.t_global_w(cap_i, cap_ave_i)#torch.Size([1, 24, 1024]), torch.Size([1, 1024])

            # local-global alignment construction
            Context_img = SCAN_attention(cap_i_expand, img_emb, smooth=9.0)#torch.Size([16, n_word, 1024]) , torch.Size([16, 3, 1024])
...
def SCAN_attention(query, context, smooth, eps=1e-8):#torch.Size([1*16, n_word, 1024]) , torch.Size([16, 3, 1024])
    """
    query: (n_context, queryL, d)
    context: (n_context, sourceL, d)
    """
    # --> (batch, d, queryL)
    queryT = torch.transpose(query, 1, 2)#torch.Size([16, 24, 1024]) -> torch.Size([16, 1024, 24])

    # (batch, sourceL, d)(batch, d, queryL)
    # --> (batch, sourceL, queryL)
    attn = torch.bmm(context, queryT)#torch.Size([16, 3, 24])
    ...
    # --> (batch, queryL, sourceL
    attn = F.softmax(attn*smooth, dim=2)#torch.Size([16, 3, 24])

    # --> (batch, sourceL, queryL)
    attnT = torch.transpose(attn, 1, 2).contiguous()#torch.Size([16, 3, 24])

    # --> (batch, d, sourceL)
    contextT = torch.transpose(context, 1, 2)#torch.Size([16, 3, 1024]) -> torch.Size([16, 1024, 3])
    # (batch x d x sourceL)(batch x sourceL x queryL)
    # --> (batch, d, queryL)
    weightedContext = torch.bmm(contextT, attnT)#torch.Size([16, 1024, 3]) * torch.Size([16, 3, 24]) ->torch.Size([16, 1024, 24])
    # --> (batch, queryL, d)
    weightedContext = torch.transpose(weightedContext, 1, 2)#torch.Size([16, 24, 1024])
